-- Batch report for multiple partner_ids (accounts) based on working single-account query
-- Replace the date and partner_id list as needed
WITH order_flags AS (
  SELECT
    op.id AS order_id,
    ...
    SUM(COALESCE(op.amount,0)) AS amount_per_order,
    SUM(CASE WHEN op.status IN ('PAY_OK','MANUAL_OK') THEN COALESCE(op.amount,0) ELSE 0 END) AS amount_success_per_order,
  ...
  WHERE op.created >= '2026-02-06 00:00:00'
    ...
    AND op.partner_id IN (
      3921,4002,3810,3673,4227,3787,3834,3725,3876,3773,3651,4042,3245,3240,3243,3244,3239,3247,3246,3035,3248,3232,3028,3234,3235,3236,3233,3021,3261,3038,1510,4039,3953,3954,3956,4190,4000,4192,4084,4211,4202,4257,4291,4043,4327,4326,4316,4276,4277,4278
    )
...
SELECT
  ...
  CASE WHEN SUM(ofl.has_create) = 0 THEN 0
       ELSE ROUND(SUM(ofl.has_success_pay) * 100.0 / SUM(ofl.has_create), 2)
  END AS conversion,
  SUM(CASE WHEN ofl.has_create = 1 AND ofl.has_pay = 0 THEN 1 ELSE 0 END) AS unfinished_cnt,
  SUM(ofl.has_create) AS total_creates,
  SUM(ofl.amount_per_order) AS turnover_all,
  SUM(ofl.amount_success_per_order) AS turnover_success,
  COALESCE(MAX(ofl.currency), '') AS currency
